...
import FriendRequests from "../models/friendRequest.model.js"; // Import the FriendRequests model for friend request operations
...
export async function getRecommendedUsers(req, res) {
  try {
    ...
    res
      .status(500)
      .json({ message: "Error fetching recommended users", error }); // Handle errors and send an error response
  }
}
...
export async function sendFriendRequest(req, res) {
  try {
    ...
    if (myId === recepientId) {
      ...
      return res
        .status(400)
        .json({ message: "You cannot send a friend request to yourself" }); // Send an error response
    }
    ...
    if (recepient.friends.includes(myId)) {
      ...
      return res
        .status(400)
        .json({ message: "You are already friends with this user" }); // Send an error response
    }
    ...
    if (existingRequests) {
      // Check if the recipient has already sent a friend request to the logged-in user
...
export async function acceptFriendRequest(req, res) {
  try {
    ...
    if (friendRequest.recepient.toString() !== myId) {
      // Check if the logged-in user is the recipient of the request
      return res.status(403).json({
        message: "You are not authorized to accept this friend request",
      }); // Send an error response if not authorized
    }
...
export async function getFriendRequests(req, res) {
  try {
    ...
    const incomingRequests = await FriendRequests.find({
      ...
      "sender",
      "name profilePicture learningLanguage nativeLanguage"
...
export async function getOutgoingFriendRequests(req, res) {
  try {
    ...
    res
      .status(500)
      .json({ message: "Error fetching outgoing friend requests", error }); // Handle errors and send an error response
  }
}
